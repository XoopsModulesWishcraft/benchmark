			...
			switch ($fct)
			{
				...
				case "save":
					...
					if (!empty($_POST[$id]['begin']))
						$test->setVar('begin', strtotime($_POST[$id]['begin']['date'])+$_POST[$id]['begin']['time']);
										
					if (!$id=$tests_handler->insert($test)) {
			...
			switch ($fct)
			{
				...
				case "list":				
					...
					foreach($results as $tid => $result) {
						$GLOBALS['xoopsTpl']->append('results', $result->toArray());
					}
					//$GLOBALS['xoopsTpl']->assign('form', benchmark_results_get_form(false));
					$GLOBALS['xoopsTpl']->assign('php_self', $_SERVER['PHP_SELF']);
					include($GLOBALS['xoops']->path('/footer.php'));
					exit(0);
			...
			switch ($fct)
			{
				...
				case "list":				
					...
					foreach($fields as $tid => $field) {
						$GLOBALS['xoopsTpl']->append('fields', $field->toArray());
					}
					$GLOBALS['xoopsTpl']->assign('form', benchmark_field_get_form(false));
					$GLOBALS['xoopsTpl']->assign('php_self', $_SERVER['PHP_SELF']);
					include($GLOBALS['xoops']->path('/footer.php'));
					exit(0);
				...
				case "save":
					...
					if (!$id=$fields_handler->insert($field)) {
