<?php
	...
	require ('header.php');
	xoops_loadLanguage('email', 'benchmark');
	error_reporting(E_ALL);
	ini_set("log_errors" , "1");
	ini_set("error_log" , dirname(__FILE__)."/errors.log.".md5(__FILE__).".txt");
	ini_set("display_errors" , "1");
	
	if ($testnumurrently = XoopsCache::read('benchmark_currently_doing_test')&&!isset($_GET['bypass']))
		exit;
	else 
	...
	echo 'Test ID: '.$tid.(isset($_SERVER['HTTP_HOST'])?'<br/>':"\n");
			...
			switch ($test->getVar('test')) {
				...
				case '_MI_BENCHMARK_INSERT':
						...
						case '_MI_BENCHMARK_TESTING':
							...
							break;
						case '_MI_BENCHMARK_CLEANUP':
							$criteria = new Criteria('tbid', '('.implode(',', $test->getVar('tbids_insert')).')', 'IN');
							$tables = $tables_handler->getObjects($criteria, true);
							foreach($tables as $tbid => $table) {
								$GLOBALS['xoopsDB']->queryF('DROP TABLE `'.$GLOBALS['xoopsDB']->prefix($table->getVar('name'))."`");
								$table->setVar('dropped', time());
				...
				case '_MI_BENCHMARK_UPDATEALL':
						...
						case '_MI_BENCHMARK_TESTING':
							...
							break;
						case '_MI_BENCHMARK_CLEANUP':
							$criteria = new Criteria('tbid', '('.implode(',', $test->getVar('tbids_updateall')).')', 'IN');
							$tables = $tables_handler->getObjects($criteria, true);
							foreach($tables as $tbid => $table) {
								$GLOBALS['xoopsDB']->queryF('DROP TABLE `'.$GLOBALS['xoopsDB']->prefix($table->getVar('name'))."`");
								$table->setVar('dropped', time());
				...
				case '_MI_BENCHMARK_ALTER':
					switch ($test->getVar('stage')) {
						...
						case '_MI_BENCHMARK_TESTING':
							...
							break;
						case '_MI_BENCHMARK_CLEANUP':
							$criteria = new Criteria('tbid', '('.implode(',', $test->getVar('tbids_alter')).')', 'IN');
							$tables = $tables_handler->getObjects($criteria, true);
							foreach($tables as $tbid => $table) {
								$GLOBALS['xoopsDB']->queryF('DROP TABLE `'.$GLOBALS['xoopsDB']->prefix($table->getVar('name'))."`");
								$table->setVar('dropped', time());
				...
				case '_MI_BENCHMARK_RENAME':
					switch ($test->getVar('stage')) {
						...
						case '_MI_BENCHMARK_TESTING':
							...
							break;
						case '_MI_BENCHMARK_CLEANUP':
							$criteria = new Criteria('tbid', '('.implode(',', $test->getVar('tbids_rename')).')', 'IN');
							$tables = $tables_handler->getObjects($criteria, true);
							foreach($tables as $tbid => $table) {
								$GLOBALS['xoopsDB']->queryF('DROP TABLE `'.$GLOBALS['xoopsDB']->prefix($table->getVar('name'))."`");
								$table->setVar('dropped', time());
